...
retained_titles = filter(events.title) do t
    ...
    isequal(title, "union square farmers market") && return true
...
unique(retained_titles)
candidate_events = filter(:title => in(retained_titles), events)
...
The following events are currently in a middle category---I'm including them for now, but may exclude them in the future:
